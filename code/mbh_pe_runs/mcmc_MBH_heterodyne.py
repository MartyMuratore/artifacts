...
MASK = False
WINDOW = True
USE_HETERODYNE = True# Toggle between heterodyned (fast) and direct (slow) likelihood
...
def lpost(params_in):
    ...
    if xp.isinf(lprior(params_in)):
        print("Prior returns -\infty")
        return -np.inf
    else:
        return llike(params_in)

print("="*80)

##=====================OPTIONAL: Uncomment to use DIRECT likelihood===================
#
# To switch from heterodyned to direct likelihood, uncomment this section:
#
# USE_HETERODYNE = False
#
# def llike(params_in):
#     """
#     Direct Whittle-based log-likelhood function (slow but exact)
#     """
#     waveform_prop_f_AE = MBH_f(wave_gen, *params_in, **kwargs)
#     # BBHx waveforms are in Hz^-1, need to divide by delta_t before IRFFT
#     waveform_prop_t_AE = xp.asarray([gap_window_array*xp.fft.irfft(waveform_prop_f_AE[k]/delta_t)
#                                       for k in range(N_channels)])
#     # Multiply by delta_t after RFFT to get back to Hz^-1
#     waveform_prop_f_AE = xp.asarray([xp.fft.rfft(waveform_prop_t_AE[k])*delta_t
#                                       for k in range(N_channels)])
#
#     diff_f_AE = [data_f_AE[k] - waveform_prop_f_AE[k] for k in range(N_channels)]
#     inn_prod = xp.asarray([inner_prod(diff_f_AE[k],diff_f_AE[k],PSD_AE[k], delta_f, xp=xp)
#                            for k in range(N_channels)])
#     return(-0.5 * xp.sum(inn_prod))
#
##====================================================================================
...
# Update filename to indicate heterodyned run
het_tag = "_heterodyne" if USE_HETERODYNE else "_direct"
# fp = mcmc_direc + f"MBH_HMs_case_1_tdi2_SNR_1137_AE_w_noise_no_mask_seed_{seed_number}{het_tag}.h5"
fp = mcmc_direc + f"MBH_HMs_case_1_tdi2_SNR_1137_AE_w_noise_w_window_seed_{seed_number}{het_tag}.h5"
