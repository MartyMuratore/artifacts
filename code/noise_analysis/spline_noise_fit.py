...
def noise_models_spritz(f,  
                        isi_rfi_back_oms_noise_level = np.sqrt( (6.35e-12)**2 + (3.32e-12)**2+ (3.0E-12)**2 ),    
                        tmi_oms_back_level = np.sqrt( (1.42e-12)**2 +(3.0E-12)**2 ),  
                        acc_level = 2.4e-15,    
                        T = armlength):
    ...
    total_noise_AA[0] = total_noise_AA[1]
...
# noise_akima_uncert=Akima_spline_psd_mod(freqs_cut,logf_knots,noise_uncert_weights)
...
parameter_noise_amplitude_array = np.array(parameter_noise_amplitude)
true_vals_spline = np.array((ndims['noise'] - 3)*[0])

true_params = np.concatenate([parameter_noise_amplitude_array, true_vals_spline])

d1 = 0.0001
d2 = 1e-4
starting_coords[:,:3] = (true_params[0:3] * (1 + d1 * np.abs(np.random.randn(nwalkers * ntemps,3))  ))

starting_coords[:,3:] = (true_params[3:] + d2 * np.random.randn(nwalkers * ntemps,ndims['noise'] - 3) )
...
10:uniform_dist(-prior_amplitude,prior_amplitude),
11:uniform_dist(-prior_amplitude,prior_amplitude),
12:uniform_dist(-prior_amplitude,prior_amplitude)}
...
if DELETE_BACKEND:
    os.system(f'rm -rf {"script_to_estimate_only_psd_spritz_noise_with_splines.h5"}')
...
def update_fn(i, res, samp):
        ...
        # Create 1x3 trace plot with colors
        parameter_labels = ['isi+rfi_OMS','tmi','TM']
        fig, axes = plt.subplots(1, 3, figsize=(15, 4))
        colors = ['#6495ed', '#ff6b6b', '#4ecdc4']  # Nice color palette

        for i in range(3):
            axes[i].plot(np.arange(len(noise_sampler)), noise_sampler[:, i], 
                        color=colors[i], alpha=0.7, linewidth=0.5)
            axes[i].set_xlabel('Iteration', fontsize=12)
            axes[i].set_ylabel(parameter_labels[i], fontsize=12)
            axes[i].grid(True, alpha=0.3)

        plt.tight_layout()
        plt.savefig("plots/noise_TM_ISI_trace.png", dpi=300)
        plt.close()
        ...
        for i in range(9):
            axes_flat[i].plot(noise_sampler[:, i+3], 
                            color=colors[i], alpha=0.7, linewidth=0.5)
        ...
        c = ChainConsumer()
        # parameter_labels = ['$isi+rfi_OMS$','$tmi$','$TM$']
        parameter_labels = ['$isi+rfi_OMS$','$tmi$','$TM$', 'knot0', 'knot1', 'knot2', 'knot3', 'knot4', 'knot5', 'knot6', 'knot7', 'knot8', 'knot9']
        ...
        for idx in indices:
            ...
            psdA = splined_psd(freqs_cut,
                            [noise_params[0], noise_params[1], noise_params[2]],
                            logf_knots, noise_params[3:8],armlength) * np.abs(h)**4
            psdE = splined_psd(freqs_cut,
                            [noise_params[0], noise_params[1], noise_params[2]],
                            logf_knots, noise_params[8:],armlength) * np.abs(h)**4
...
def log_like_fn(x,data,df,freqs,filter_tf,subset = 1):
        ...
        for i in range(len(inds) - 1):
            ...
            psdA_estimated = splined_psd(freqs, [noise_params[0], noise_params[1], noise_params[2]],logf_knots,noise_params[3:8],armlength)
            psdE_estimated = splined_psd(freqs, [noise_params[0], noise_params[1], noise_params[2]],logf_knots,noise_params[8:],armlength)
...
ensemble = EnsembleSampler(
        ...
        pool = pool,
        update_iterations=100, # to use the update function
